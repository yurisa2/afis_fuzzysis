PATH <- "C:/Bitnami/wampstack-7.1.20-1/apache2/htdocs/afis_fuzzysis"
setwd(PATH)

options(warn=-1) # Comment for debug, uncomment for production (supress warnings) # EDITAVEL

library("parallel")
library("doSNOW") # Works for windows, in Linux/MacOS, another package

cl <- makeCluster(4) #change to your number of CPU cores used in the cluster
registerDoSNOW(cl)

source(file="include/functions.R")

arq <- list.files(path = "data", pattern = "*.csv")

list_full <- NULL

foreach(file_idx=arq) %dopar% {

data <- read.csv(paste0("data/",file_idx))


# Simple Feature Engineering
direcao_1 <- data[which(data$direcao == 1),]
direcao_1$lucro <- ifelse (direcao_1$lucro>0,1,0)
direcao_1$lucro <- factor(direcao_1$lucro)

# Get Col Numbers by name
# print(col_names_func(direcao_1))


possb_feat <- c(7:72) # To work with auto select features, define the possible features (col numbers)
nbin <- 6 # Target binary column (factor 0;1)


################################################################################

print(paste("Start!:",Sys.time()))

# res_mat20 <- evaluate_afis(20,(nrow(training_data)-2000),training_data,possb_feat)
# res_mat40 <- evaluate_afis(40,(nrow(training_data)-2000),training_data,possb_feat)
# res_mat60 <- evaluate_afis(60,(nrow(training_data)-2000),training_data,possb_feat)
# res_mat80 <- evaluate_afis(80,(nrow(training_data)-2000),training_data,possb_feat)
res_mat100 <- evaluate_afis(100,(nrow(direcao_1)-100),direcao_1,possb_feat)


list_line <- c(file_idx,res_mat100$byClass[3],res_mat100$byClass[4])

list_full <- rbind(list_full,list_line)

}

stopCluster(cl) # Free Memory and some cycles

write.csv(list_full,file="compra_multiCPU.csv")
